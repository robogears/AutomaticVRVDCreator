using AutoVRVD.Detection;
using AutoVRVD.Display;
using AutoVRVD.Interop;
using AutoVRVD.UI;
using AutoVRVD.Update;
...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        if (!createdNew)
        {
            MessageBox.Show("AutoVRVD is already running — look for its icon in the system tray.",
                "AutoVRVD", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        void OnWatchdogFail()
        {
            ...
            tray.Notify("AutoVRVD", "SUDOVDA watchdog failed; the virtual display may have been removed.", ToolTipIcon.Warning);
        }
        ...
        tray.CreateDisplayRequested += () => Task.Run(() =>
        {
            try
            {
                ...
                if (session.Activate(out string? name))
                {
                    ...
                    tray.Notify("AutoVRVD", $"Virtual display active: {name}.");
                }
                else
                {
                    tray.SetStatus(AppStatus.Error, "Activate failed");
                    tray.Notify("AutoVRVD", "Failed to activate the virtual display (see logs).", ToolTipIcon.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        tray.RemoveDisplayRequested += () => Task.Run(() =>
        {
            try
            {
                ...
                tray.Notify("AutoVRVD", ok ? "Displays restored; virtual display removed." : "Restore had issues (see logs).",
        ...
        tray.RunDiagnosticsRequested += () => Task.Run(() =>
        {
            try
            {
                tray.Notify("AutoVRVD", "Detection diagnostics started (60s). Connect, wait, then disconnect your headset now.");
                string report = DetectionDiagnostics.Run(cfg, 60);
                tray.Notify("AutoVRVD", "Diagnostics complete — opening report.");
