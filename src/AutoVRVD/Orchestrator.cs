    ...
    private void OnConnected()
    {
        lock (_gate)
        {
            ...
            if (_cfg.SkipIfApolloActive && GdiInterop.HasActiveDisplayMatching("SudoMaker"))
            {
                ...
                _tray.Notify("AutoVRVD", "VR connected, but a virtual display is already active (Apollo?). Skipping.", ToolTipIcon.Warning);
                return;
            }
            ...
            if (_session.Activate(out var name))
            {
                ...
                _tray.Notify("AutoVRVD", $"VR connected — {_cfg.Display} virtual display active.");
            }
            else
            {
                _tray.SetStatus(AppStatus.Error, "Activation failed");
                _tray.Notify("AutoVRVD", "VR connected but activation failed (see logs).", ToolTipIcon.Error);
            }
        }
    }
