...
public sealed class DisplaySnapshot
{
    ...
    public void SaveToDisk()
    {
        try
        {
            AutoVRVD.Paths.EnsureCreated();
            File.WriteAllText(AutoVRVD.Paths.StatePath, JsonSerializer.Serialize(this, JsonOpts));
            Log.Info($"Persisted session state ({NumPaths} paths) to {AutoVRVD.Paths.StatePath}.");
        }
    ...
    public static DisplaySnapshot? LoadFromDisk()
    {
        try
        {
            return File.Exists(AutoVRVD.Paths.StatePath)
                ? JsonSerializer.Deserialize<DisplaySnapshot>(File.ReadAllText(AutoVRVD.Paths.StatePath))
                : null;
        }
    ...
    {
        try { if (File.Exists(AutoVRVD.Paths.StatePath)) File.Delete(AutoVRVD.Paths.StatePath); }
